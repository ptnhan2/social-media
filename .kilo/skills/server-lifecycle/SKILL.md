...
1. **Khởi động**: background process tool (persistent: true cho thứ cần sống qua session). Command đúng:
   ...
   - LangGraph agent: `harness\.venv\Scripts\python.exe -m langgraph_cli dev --port 2025 --host 127.0.0.1` (cần env `PYTHONUTF8=1` — không có nó sẽ crash emoji logging trên Windows)
