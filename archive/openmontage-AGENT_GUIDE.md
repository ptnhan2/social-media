...
| `longform-educational` | 10-15 minute educational videos, chapter-segmented renders | beta |
...
### Vox-style editorial explainer → `skills/meta/vox-editorial-pipeline.md`

For any brief that wants a **Vox-style editorial explainer** (paper-collage world, kinetic typography with yellow highlight sweeps, archival photos cut out along the subject, karaoke subtitles — the Johnny Harris / "AI Can't Write Subtext" look), read **`skills/meta/vox-editorial-pipeline.md`** BEFORE writing any scene. It is a **style + reusable engine**: it routes to the shared pipeline folder (`docs/vox-pipeline/` — playbook + automated quality gate), the reusable Remotion primitives (`voxKit.tsx`), and the user-approved visual rules. The quality gate (`docs/vox-pipeline/gate_vox.py`) is the review — run it before every deliverable; 0 FAIL required. Do not improvise the visual language; it was approved after ~30h of iteration.
