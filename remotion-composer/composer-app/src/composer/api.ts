...
export type ScriptBeatOp = "set-transcript" | "set-direction" | "add" | "delete" | "move" | "set-treatment" | "set-duration";
export const scriptBeat = (projectId: string, op: ScriptBeatOp, payload: { beatId?: string; text?: string; dir?: "up" | "down"; index?: number; treatment?: string; duration?: number } = {}) => requestJson<{ ok: true; projectId: string; op: string; beats: number; durationSec: number }>(`/api/project/script-beat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, op, ...payload }) });
