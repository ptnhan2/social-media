...
import { fontStack, resolveFontFamily } from "./fontFaces";
import {
...
const DiagramNodeView: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ color, fontFamily: fontStack("body"), fontSize: getStyle<number>("treatments.semantic-diagram.node.fontSize", 20), fontWeight: getStyle<number>("treatments.semantic-diagram.node.fontWeight", 900), letterSpacing: "0.04em", lineHeight: 1.44, textTransform: "uppercase", textShadow: "0 2px 6px rgba(0,0,0,0.7)" }}>
          {node.label}
        </div>
        {node.detail ? <div style={{ color: PAPER(), fontFamily: fontStack("body"), fontSize: getStyle<number>("treatments.semantic-diagram.node.detailFontSize", 13), lineHeight: 1.35, marginTop: 6, opacity: 0.78, fontWeight: 900 }}>{node.detail}</div> : null}
      </div>
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BLACK(), color: PAPER(), overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", left: 86, top: 62, opacity: titleIn, transform: `translateY(${(1 - titleIn) * 18}px)` }}>
        {(kicker ?? narrativeLabel) ? <div style={{ color: CYAN(), fontFamily: fontStack("body"), fontSize: getStyle<number>("treatments.semantic-diagram.kicker.fontSize", 18), fontWeight: getStyle<number>("treatments.semantic-diagram.kicker.fontWeight", 900), letterSpacing: getStyle<string>("treatments.semantic-diagram.kicker.letterSpacing", "0.18em"), textTransform: "uppercase", marginBottom: 12, lineHeight: 1.4222, textShadow: `0 2px 8px rgba(0,0,0,0.7)` }}>{kicker ?? narrativeLabel}</div> : null}
        <div style={{
          color: PAPER(),
...
export const ScreenProofInWorld: React.FC<ScreenProofInWorldProps> = ({
  ...
  return (
    <AbsoluteFill style={{ background: BLACK(), overflow: "hidden" }}>
      ...
      {caption ? <div style={{ position: "absolute", left: "8%", right: "8%", bottom: "5%", color: PAPER(), fontFamily: fontStack("body"), fontSize: getStyle<number>("treatments.screen-proof.caption.fontSize", 28), fontStyle: "italic", fontWeight: 900, textAlign: "center", textShadow: "0 3px 12px #000, 0 0 18px rgba(0,0,0,0.7)" }}>{caption}</div> : null}
    </AbsoluteFill>
  );
};
...
export const HostReflectionShot: React.FC<HostReflectionShotProps> = ({ src, subtitle, accent = AMBER(), lightSide = "left" }) => {
  ...
  const subFont = resolveFontFamily(getStyle<string>("treatments.host-reflection.subtitle.fontFamily", "editorial"));
...
export const AudienceDemandProof: React.FC<AudienceDemandProofProps> = ({
  ...
  return (
    <AbsoluteFill style={{ background: BLACK(), overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", left: 70, top: 54, color: accent, fontFamily: fontStack("body"), fontSize: getStyle<number>("treatments.audience-demand.kicker.fontSize", 18), fontWeight: getStyle<number>("treatments.audience-demand.kicker.fontWeight", 900), letterSpacing: "0.18em", textTransform: "uppercase", opacity: 0.9, textShadow: "0 2px 8px rgba(0,0,0,0.7)" }}>audience demand</div>
      {comments.map((comment, index) => {
        ...
        return (
          <div key={`${comment.text}-${index}`} style={{ position: "absolute", left: `${x}%`, top: `${y}%`, width: 540, transform: `translate(-50%, -50%) rotate(${rotation}deg) translateY(${(1 - entrance) * 24}px) scale(${0.94 + entrance * 0.06})`, opacity: entrance * (contextIn ? 0.66 : 1), zIndex: index + 2 }}>
            <div style={{ background: "rgba(248,248,244,.96)", color: "#14171c", borderRadius: 8, padding: "18px 24px", boxShadow: `0 12px 30px rgba(0,0,0,.38), 0 0 18px ${comment.color ?? accent}30`, fontFamily: fontStack("body"), fontSize: getStyle<number>("treatments.audience-demand.comments.fontSize", 25), lineHeight: 1.2, fontWeight: getStyle<number>("treatments.audience-demand.comments.fontWeight", 900), textShadow: "0 1px 2px rgba(0,0,0,0.25)" }}>
              {comment.text}
            </div>
      ...
      {caption ? <div style={{ position: "absolute", left: "8%", right: "8%", bottom: "6%", color: "#f4e8cf", fontFamily: fontStack("editorial"), fontStyle: "italic", fontSize: getStyle<number>("treatments.audience-demand.caption.fontSize", 28), fontWeight: 900, textAlign: "center", textShadow: "0 3px 12px #000, 0 0 20px rgba(0,0,0,0.7)", opacity: responseIn }}>{caption}</div> : null}
    </AbsoluteFill>
  );
};
...
export const ProcessTimeline: React.FC<ProcessTimelineProps> = ({ title, steps, activeStep = 0, accent = AMBER(), narrativeLabel, presence }) => {
  ...
  return (
    <AbsoluteFill style={{ background: BLACK(), color: PAPER(), overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", left: 76, top: 62, opacity: titleIn, transform: `translateY(${(1 - titleIn) * 18}px)`, fontFamily: fontStack("body") }}>
        <div style={{ color: accent, fontSize: getStyle<number>("treatments.process-timeline.kicker.fontSize", 17), fontWeight: getStyle<number>("treatments.process-timeline.kicker.fontWeight", 900), letterSpacing: "0.18em", lineHeight: 1.4222, textTransform: "uppercase", textShadow: "0 2px 8px rgba(0,0,0,0.7)" }}>{narrativeLabel ?? "workflow"}</div>
        <div style={{ fontFamily: fontStack("display"), fontSize: getStyle<number>("treatments.process-timeline.title.fontSize", 48), fontWeight: getStyle<number>("treatments.process-timeline.title.fontWeight", 900), lineHeight: 1.4222, marginTop: 12, background: `linear-gradient(135deg, ${getStyle<string>("colors.gradientStart", "#ff6b35")} 0%, ${getStyle<string>("colors.gradientEnd", "#ffd166")} 55%, ${getStyle<string>("colors.gradientStart", "#ff6b35")} 100%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", filter: `drop-shadow(0 3px 10px rgba(0,0,0,0.7)) drop-shadow(0 0 16px ${accent}40)` }}>{title}</div>
      </div>
      ...
      {steps.map((step, index) => {
        ...
        return (
          <div key={`${step.label}-${index}`} style={{ position: "absolute", left: `${x}%`, top: "46%", width: 180, transform: `translate(-50%, -50%) translateY(${(1 - reveal) * 28}px)`, opacity: reveal }}>
            ...
            <div style={{ marginTop: 18, padding: "12px 14px", border: `2px solid ${selected ? color : `${color}88`}`, borderRadius: 8, background: selected ? `${color}18` : "rgba(7,9,13,.76)", textAlign: "center", fontFamily: fontStack("body") }}>
              <div style={{ color, fontSize: getStyle<number>("treatments.process-timeline.step.fontSize", 16), fontWeight: getStyle<number>("treatments.process-timeline.step.fontWeight", 900), textTransform: "uppercase", letterSpacing: "0.04em", lineHeight: 1.4222, textShadow: "0 2px 8px rgba(0,0,0,0.8)" }}>{step.label}</div>
              {step.detail ? <div style={{ color: PAPER(), opacity: 0.7, fontSize: 12, lineHeight: 1.3, marginTop: 6, fontWeight: 900 }}>{step.detail}</div> : null}
            </div>
          </div>
        );
      })}
      <div style={{ position: "absolute", right: 76, bottom: 48, color: "rgba(244,232,207,.5)", fontFamily: fontStack("body"), fontSize: 14, lineHeight: 1.2, letterSpacing: "0.08em", textTransform: "uppercase" }}>step {active + 1} / {steps.length}</div>
      {presence ? <CharacterPresence config={presence} accent={accent} /> : null}
    </AbsoluteFill>
  );
};
...
export const CandidateComparison: React.FC<CandidateComparisonProps> = ({ title, candidates, selectedIndex = 0, criteria, accent = AMBER() }) => {
  ...
  return (
    <AbsoluteFill style={{ background: BLACK(), color: PAPER(), overflow: "hidden" }}>
      <div style={{ position: "absolute", left: 70, top: 54, opacity: titleIn, transform: `translateY(${(1 - titleIn) * 18}px)`, fontFamily: fontStack("body") }}>
        <div style={{ color: accent, fontSize: getStyle<number>("treatments.candidate-comparison.kicker.fontSize", 17), fontWeight: getStyle<number>("treatments.candidate-comparison.kicker.fontWeight", 900), letterSpacing: "0.18em", lineHeight: 1.4222, textTransform: "uppercase", textShadow: "0 2px 8px rgba(0,0,0,0.7)" }}>compare</div>
        <div style={{ fontFamily: fontStack("display"), fontSize: getStyle<number>("treatments.candidate-comparison.title.fontSize", 44), fontWeight: getStyle<number>("treatments.candidate-comparison.title.fontWeight", 900), lineHeight: 1.4222, marginTop: 10, background: `linear-gradient(135deg, ${gradientStart} 0%, ${gradientEnd} 55%, ${gradientStart} 100%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", filter: "drop-shadow(0 3px 10px rgba(0,0,0,0.7))" }}>{title}</div>
          ...
          return (
            <div key={`${candidate.label}-${index}`} style={{ opacity: entrance, transform: `translateY(${(1 - entrance) * 24}px) scale(${selected ? 1 : 0.96})`, border: `2px solid ${selected ? color : `${color}55`}`, borderRadius: 10, background: selected ? `${color}18` : "rgba(7,9,13,.76)", boxShadow: selected ? `0 0 24px ${color}44` : "none", overflow: "hidden", display: "flex", flexDirection: "column" }}>
              <div style={{ height: "68%", background: "#11151b", position: "relative" }}>
                {candidate.src ? <Img src={candidate.src} style={{ width: "100%", height: "100%", objectFit: "cover", filter: selected ? "none" : "grayscale(.65) brightness(.7)" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color, fontFamily: fontStack("body"), fontSize: getStyle<number>("treatments.candidate-comparison.candidate.fontSize", 18) }}>NO PREVIEW</div>}
                <div style={{ position: "absolute", top: 12, left: 12, color: selected ? color : PAPER(), fontFamily: fontStack("body"), fontSize: 13, fontWeight: 900, letterSpacing: "0.12em", textTransform: "uppercase", background: "rgba(0,0,0,.68)", padding: "5px 8px", borderRadius: 4, textShadow: "0 1px 4px rgba(0,0,0,0.8)" }}>{selected ? "selected" : "alternative"}</div>
              </div>
              <div style={{ padding: "14px 16px", fontFamily: fontStack("body") }}>
  ...
  return (
    <AbsoluteFill style={{ background: BLACK(), overflow: "hidden", opacity: entrance }}>
      ...
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, minHeight: "16%", background: BLACK(), display: "flex", alignItems: "center", justifyContent: "center", padding: "0 9%" }}>
        {subtitle ? <div style={{ color: accent, fontFamily: fontStack("editorial"), fontStyle: "italic", fontSize: getStyle<number>("treatments.cinematic-metaphor.subtitle.fontSize", 28), fontWeight: 900, textAlign: "center", textShadow: "0 3px 12px #000, 0 0 20px rgba(0,0,0,0.7)" }}>{subtitle}</div> : null}
      </div>
    </AbsoluteFill>
  );
};
