...
const addAudioPlanClips = (doc: IsaacVerseEditDoc, tracks: Map<string, EditorTrack>) => {
  ...
  for (const segment of doc.audioPlan?.voice ?? []) {
    ...
    addClip(tracks, "voice", clip({ id: `clip:voice:${segment.id}`, kind: "voice", trackId: "voice", range: { startSec: segment.startSec, endSec: segment.endSec }, label: "Voiceover", source: segmentBeat ? { beatId: segmentBeat.id } : {}, color: "cyan", metadata: { src: segment.src, transcript: segment.transcript, sentenceText: segment.transcript, providerText: buildProviderText(segment.transcript), segmentId: segment.id, ...(segment.qc ? { qc: segment.qc } : {}), ...(segment.takeId ? { takeId: segment.takeId } : {}), ...(segment.takes ? { takes: segment.takes } : {}) } }));
  }
