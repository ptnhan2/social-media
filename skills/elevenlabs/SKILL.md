...
|-------|---------|--------------|-------|
| `eleven_v3` | **Latest & best** | None | **Default.** 74 languages. Best Vietnamese and general multilingual quality. Requires `language` param for non-English. |
| `eleven_multilingual_v2` | Highest consistency (older gen) | None | Stable, 29 languages. Use only if v3 unavailable for the voice. |
...
**Choose:** v3 is the DEFAULT for all narration — quality is best, including Vietnamese. Use flash/turbo for SSML control, multilingual_v2 only as a fallback.

### Non-English Narration (CRITICAL — language parameter)

**When the narration language is NOT English (e.g. Vietnamese, Chinese, Japanese, Spanish), you MUST pass the `language` parameter with the ISO-639-1 code** (e.g. `"language": "vi"`, `"language": "zh"`).

- Without it, the multilingual model **auto-detects and often falls back to English prosody** — the read comes out with a foreign accent and unnatural rhythm (the #1 complaint on Vietnamese narration).
- **Model matters as much as the parameter:** `eleven_v3` (the default) produces natural Vietnamese with `language="vi"`; older `eleven_multilingual_v2` produces noticeably worse Vietnamese even with the parameter.
- Only multilingual models (`eleven_v3`, `eleven_multilingual_v2`, `eleven_flash_v2_5`) accept `language`; single-language models ignore it.
- Example:
  ```python
  result = client.text_to_speech.convert(
      text="Cuốn sách bạn đang đọc, chỉ là một phần tám của tảng băng.",
      voice_id="hfokZ662h0bgEIMgFGCx",
      model_id="eleven_v3",
      language="vi",
      voice_settings=VoiceSettings(stability=0.6, similarity_boost=0.75, style=0.3, speed=0.95),
  )
  ```
- In OpenMontage, the `elevenlabs_tts` tool accepts `language` in its input schema and defaults `model_id` to `eleven_v3`. The `tts_selector` routes `language` through when the provider supports it. Always set it for non-English scripts.

**Voice selection for non-English:** check the voice's `verified_languages` via `client.voices.get_all()` or `/v1/voices/{id}`. A voice with `vi` in `verified_languages` (or `locale: vi-VN`) is verified for Vietnamese. Most popular premade voices work well with v3 + `language="vi"`. **Always generate a 10-15s sample and have it approved before batch narration.**
