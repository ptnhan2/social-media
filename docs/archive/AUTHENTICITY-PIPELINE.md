...
> Core principle: OPTIMIZE the existing stack (ElevenLabs default + HyperFrames), NOT replace it. [Open Design ĐÃ BỎ 2026-07-27]
...
## 2. HyperFrames Pipeline

### HyperFrames: DESIGN.md = Brand Contract
...
### HyperFrames: Validation (replaces OD anti-slop)

- `hyperframes check`: lint + runtime + layout + motion + contrast (WCAG AA) — must pass before render.
- `hyperframes inspect`: headless Chrome seeks timeline, maps overflow/layout issues with timestamps + fix hints.
- `hyperframes validate`: WCAG contrast audit (5 timestamps, samples bg behind text).
- Animation map: per-tween summaries + dead-zone detection (verify choreography).
- Humanizer (separate skill): strips AI tells trên SCRIPT (em-dashes, "moreover," "furthermore") — áp dụng ở phase 1, không phải HyperFrames.
...
3. Pass durations to HyperFrames composition → root `data-duration` + clip `data-start/data-duration` MATCH audio
...
### ⚠️ Stance: Host INSIDE vs Essayist OUTSIDE (THE FOUNDATION — check first, trước 9 techniques)

> Tất cả 9 techniques bên dưới FAIL nếu stance sai. Stance = vị trí người nói, quyết định TRƯỚC mọi kỹ thuật. Đây là lỗi TONE lặp lại nhiều nhất — rule "first-person analytical, experiential, not secondhand" KHÔNG ĐỦ vì "first-person" vẫn có thể essay (analytical, observer).

**KTTV (web5ngay) đứng BÊN TRONG trải nghiệm** — "mình" là người đang trải qua, "chúng mình/chúng ta" kéo viewer vào đồng hành. Host là **nhân vật trong story** (bực, bí, tìm ra, chia sẻ). Viewer = companion.

**Essayist (AVOID) đứng BÊN NGOÀI** — phân tích "The AI thì thế", "The model trained thế". Dù dùng "I" để intro, phần lớn là analyst quan sát specimen, viewer = audience bị giảng.

| | KTTV (TARGET) | Essayist (AVOID) |
|---|---|---|
| Vị trí host | BÊN TRONG struggle | BÊN NGOÀI, giải thích |
| Chủ ngữ chính | mình / chúng mình (EN: I/we/you) | The AI / The model / It |
| Quan hệ viewer | companion | audience |
| Discovery | cá nhân, vừa trải qua | phân tích người khác |

**Stance diagnostic (bắt buộc trước save script):** đọc 3 đoạn ngẫu nhiên. Đếm chủ ngữ main clause. Nếu đa số "The AI/It/This" (outside) → FAIL → rewrite "I/We/You" (inside). Host phải là character trong struggle, viewer là companion.

**BAN essay-stance markers làm move chính:** "Most people think X", "The AI does Y", "Here's why", "The model was trained to Z". Thay bằng lived moment của host.

**Language note:** content English nên "mình"→"I", "chúng mình/chúng ta"→"we/you" — stance transfer, pronoun localize.
...
3. VOICEOVER + VISUALS + RENDER (automated via HyperFrames)
   ...
   ├── HyperFrames + DESIGN.md: author composition per scene matching narration
