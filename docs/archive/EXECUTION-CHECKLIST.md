...
- [ ] **2.1** **ElevenLabs** — voiceover TTS. Settings: stability 35-45%, similarity 75-80%, style 0-15% (see `AUTHENTICITY-PIPELINE.md`). Consistent voice = brand identity. (Copy key vào `.env`)
- [ ] **2.2** **OpenMontage** — video production system (agentic, 12 pipelines, 100+ tools). Clone tại `openmontage/`, setup `.venv` + requirements + remotion-composer + `.env`. Đọc `openmontage/AGENT_GUIDE.md` trước khi dùng. [MIGRATED 2026-08-01 — thay HyperFrames-handdraw]
- [ ] **2.3** **CapCut** — video assembly + auto-captions (Montserrat Bold, stroke 15px) + film grain overlay + 90% hard cuts + music -22dB + normalize -14 LUFS (see `AUTHENTICITY-PIPELINE.md`) — dùng cho post-edit nếu cần
...
## Phase 3: Verify OpenMontage (video production layer)

- [ ] **3.1** Verify setup: `openmontage\.venv\Scripts\python.exe -c "from tools.tool_registry import registry; registry.discover(); print('OK')"` (chạy trong `openmontage/`)
- [ ] **3.2** Verify preflight: chạy `provider_menu_summary()` → xem "X of Y configured" (ElevenLabs TTS + Pixabay/Pexels stock + Piper local + FFmpeg = đủ cho video cơ bản)
- [ ] **3.3** Test 1 pipeline: chọn script có sẵn (`videos/2026-08/01-subtext/03-script.md`) → init project → chạy stage → render → final_review PASS
- [ ] **3.4** Brand: `docs/DESIGN.md` (literary cream/serif) — map thành custom playbook `openmontage/styles/` khi cần
- [ ] ~~3.5-3.6 Open Design / HyperFrames-handdraw setup~~ **REMOVED 2026-08-01** (chuyển hẳn OpenMontage — xem `STACK-DECISIONS.md` §2)
...
| `DESIGN.md` | Literary brand system (9-section, cho HyperFrames brand reference) |
